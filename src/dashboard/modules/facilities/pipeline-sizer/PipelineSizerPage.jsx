...
import { 
  ...
  History, FolderOpen, Share2, Maximize2, Minimize2
} from 'lucide-react';
...
import { Step5Integrity, Step6Operations, Step7Sensitivity, Step8Optimization, Step9Reporting } from './components/AdvancedSteps';

// Hooks & Utils
import { usePipelineCalculation } from './hooks/usePipelineCalculation';
import { DEFAULT_FLUID_PROPS } from './utils/constants';

const PipelineSizerPage = () => {
  const { toast } = useToast();
  const { user } = useAuth();
  const { setIsFullscreenApp, setShowMainSidebar, setShowMainHeader } = useApplicationLayout();
  const { runFullAnalysis, runSensitivity, isCalculating, results, sensitivityResults, setResults } = usePipelineCalculation();
  
  // Layout Management - Fullscreen Mode Logic
  useEffect(() => {
    // Enable Fullscreen Mode on Mount
    setIsFullscreenApp(true);
    setShowMainSidebar(false);
    setShowMainHeader(false); // Also hide header for true fullscreen

    // Restore Default Layout on Unmount
    return () => {
      setIsFullscreenApp(false);
      setShowMainSidebar(true);
      setShowMainHeader(true);
    };
  }, [setIsFullscreenApp, setShowMainSidebar, setShowMainHeader]);
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 bg-[#0F172A] text-slate-100 flex flex-col font-sans z-[100] overflow-hidden">
         {/* Enhanced Header */}
         <div className="h-16 bg-slate-900 border-b border-slate-700 flex items-center justify-between px-6 z-50 shadow-md shrink-0">
            ...
            <div className="flex items-center gap-2">
                 <Dialog open={isLoadModalOpen} onOpenChange={setIsLoadModalOpen}>
                    <DialogTrigger asChild><Button variant="outline" size="sm" className="bg-slate-800 border-slate-700 hover:bg-slate-700 hidden sm:flex"><FolderOpen className="w-4 h-4 mr-2"/> Cases</Button></DialogTrigger>
                 ...
                 <Button variant="outline" size="sm" onClick={handleSaveCase} className="bg-slate-800 border-slate-700 hover:bg-slate-700"><Save className="w-4 h-4 mr-2 sm:mr-0 md:mr-2"/><span className="hidden md:inline">Save</span></Button>
                 
                 <Dialog open={isAuditModalOpen} onOpenChange={setIsAuditModalOpen}>
                    <DialogTrigger asChild><Button variant="outline" size="sm" onClick={() => currentCaseId && loadAuditAndReports(currentCaseId)} disabled={!currentCaseId} className="bg-slate-800 border-slate-700 hover:bg-slate-700 disabled:opacity-50"><History className="w-4 h-4 mr-2 sm:mr-0 md:mr-2"/><span className="hidden md:inline">History</span></Button></DialogTrigger>
         ...
         <main className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-slate-700 bg-slate-950/50 pb-24">
         ...
         {/* Sticky Footer */}
         <div className="h-20 bg-[#1e293b] border-t-2 border-slate-700 flex items-center justify-between px-8 z-50 shadow-[0_-5px_20px_rgba(0,0,0,0.5)] shrink-0">
             <Button 
                variant="outline" 
             ...
             <div className="flex items-center gap-4 bg-slate-900/50 py-2 px-6 rounded-full border border-slate-700/50 hidden md:flex">
                 <Input 
                    value={caseMeta.caseName} 
                    onChange={e => setCaseMeta({...caseMeta, caseName: e.target.value})} 
                    className="bg-transparent border-none h-8 w-64 text-center focus-visible:ring-0 text-white font-medium focus:bg-slate-800/50"
             ...
             <Button 
                ...
                className={`
                    ...
                    ${currentStep === 2 ? 'bg-indigo-600 hover:bg-indigo-500' : 
                      (currentStep >= 3 && !selectedSize && currentStep !== steps.length - 1) ? 'bg-slate-700 text-slate-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500'}
                    text-white
                `}
                disabled={currentStep === 2 ? isCalculating : (currentStep >= 3 && !selectedSize && currentStep !== steps.length - 1)}
             >
